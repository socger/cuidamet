...
import CreateProfile_PersonalInfo from "./CreateProfile_PersonalInfo";
...
interface CreateProfile_OfferServiceProps {
...
const CreateProfile_OfferService: React.FC<CreateProfile_OfferServiceProps> = ({
  onComplete,
  onCancel,
  initialData,
  currentView = "offer",
      ...
      <div className="space-y-6 animate-slide-up">
        ...
        >
          <div className="flex items-center">
            <div
              className={`p-2 rounded-full bg-white/50 mr-3 ${catInfo?.color}`}
            >
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div className="px-4 pb-4 space-y-4">
            {completedServices.map((cat) => {
              ...
              const activeVariations = config.variations.filter(
                (v) => v.enabled
              );
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-center mb-2">
                    <div className="flex items-center">
                      <catInfo.icon
                        className={`w-5 h-5 mr-2 ${catInfo.color}`}
                      />
                    ...
                    <span className="font-bold text-teal-600 text-sm">
                      Desde{" "}
                      {Math.min(
                        ...(activeVariations.map((v) => v.price) || [0])
                      )}
                      €
                    </span>
                  </div>
...
export default CreateProfile_OfferService;

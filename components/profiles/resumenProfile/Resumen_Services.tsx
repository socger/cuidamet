  ...
  return (
    <div className="px-4 pb-4 space-y-4">
      {completedServices.map((cat) => {
        ...
        // Determinar si el icono es un componente o una ruta SVG
        const IconComponent = typeof catInfo.icon !== 'string' ? catInfo.icon : null;
        const iconSrc = typeof catInfo.icon === 'string' ? catInfo.icon : null;
          ...
          >
            <div className="flex justify-between items-center mb-2">
              <div className="flex items-center">
                {IconComponent ? (
                  <IconComponent className={`w-5 h-5 mr-2 ${catInfo.color}`} />
                ) : iconSrc ? (
                  <img 
                    src={iconSrc} 
                    alt={catInfo.label} 

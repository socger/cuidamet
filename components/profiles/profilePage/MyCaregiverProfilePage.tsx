...
const defaultDashboardData = {
  ...
  reviewsCount: 0,
  upcomingJobs: [],
  activeServices: [] as { id: CareCategory, label: string, status: string, price: number }[]
};
...
const SettingsItem: React.FC<SettingsItemProps> = ({ icon, label, onClick, isDestructive, subLabel }) => (
    <button
      onClick={onClick}
      className={`w-full flex items-center justify-between p-4 text-left transition-colors border-b border-slate-100 last:border-0 hover:bg-slate-50 ${isDestructive ? 'text-red-600' : 'text-slate-700'}`}
    >
      <div className="flex items-center">
        <div className={`${isDestructive ? 'text-red-500' : 'text-slate-500'} mr-4`}>{icon}</div>
        <div>
            <span className="font-medium block">{label}</span>
            {subLabel && <span className="text-xs text-slate-400 font-normal">{subLabel}</span>}
        </div>
      </div>
      <ChevronRightIcon className={`w-5 h-5 ${isDestructive ? 'text-red-300' : 'text-slate-400'}`} />
    </button>
);
  ...
  return (
    <div className="bg-slate-50 min-h-screen flex flex-col">
      <PageHeader 
        ...
        onBack={onBack} 
        rightAction={
            <button onClick={() => onNavigateEditProfile(null)} className="text-teal-600 font-semibold text-sm">
                Editar
            </button>
        }
      />
      ...
      <main className="container mx-auto px-4 py-6 pb-28 flex-grow overflow-y-auto">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-6">
            ...
            <div className="bg-gradient-to-r from-teal-500 to-teal-600 pt-8 pb-6 px-6 relative flex flex-col items-center text-center">
                ...
                <div className="flex items-center text-sm text-teal-50 mt-1 mb-3">
                    <MapPinIcon className="w-4 h-4 mr-1 opacity-80" /> {displayData.location}
                </div>
            ...
            <div className="px-5 py-5">
                <div className="flex justify-between items-center mb-3">
                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Mis Detalles</h3>
                </div>
                
                <div className="space-y-4">
                    <div>
                        <p className="text-xs text-slate-500 mb-1.5 font-medium">Idiomas</p>
                        <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
                            {displayData.languages.length > 0 ? displayData.languages.map(lang => (
                                <span key={lang} className="px-2.5 py-1 bg-slate-100 text-slate-600 text-xs rounded-lg border border-slate-200 font-medium">
                                    {lang}
                                </span>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between relative overflow-hidden">
                ...
                <p className="text-xs font-medium text-slate-500 uppercase">Ingresos Mes</p>
                <p className="text-2xl font-bold text-slate-800 mt-1">{displayData.earningsMonth}€</p>
            </div>
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
                 <p className="text-xs font-medium text-slate-500 uppercase">Valoración</p>
                 <div className="flex items-center mt-1">
                    <span className="text-2xl font-bold text-slate-800 mr-2">{displayData.rating.toFixed(1)}</span>
                    <div className="flex">
        ...
        <div className="mb-6">
            ...
            >
                <div className="flex items-center relative z-10">
                    <div className={`p-3 rounded-full mr-4 ${isPremium ? 'bg-white/20 text-white' : 'bg-amber-50 text-amber-500'}`}>
                        <StarIcon className="w-6 h-6" />
                    </div>
                    <div className="text-left">
                        <h3 className={`font-bold text-lg ${isPremium ? 'text-white' : 'text-slate-800'}`}>
                            {isPremium ? 'Cuidamet PRO Activo' : 'Hazte Cuidamet PRO'}
                        </h3>
                        <p className={`text-sm ${isPremium ? 'text-white/90' : 'text-slate-500'}`}>
                            {isPremium ? 'Tu perfil está destacado.' : 'Consigue hasta 3x más contactos.'}
                        </p>
                    </div>
                </div>
                <div className={`p-2 rounded-full ${isPremium ? 'bg-white/20 text-white' : 'bg-slate-50 text-slate-400 group-hover:text-amber-500 group-hover:bg-amber-50'} transition-colors relative z-10`}>
                    <ChevronRightIcon className="w-5 h-5" />
                </div>
                
                {/* Decorative background for free users */}
                {!isPremium && <div className="absolute right-0 top-0 bottom-0 w-1/3 bg-gradient-to-l from-amber-50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>}
            </button>
        </div>
        ...
        <section className="mb-8">
            <div className="flex items-center justify-between mb-3 px-1">
                <h3 className="font-bold text-slate-800">Mis Servicios y Tarifas</h3>
                <button 
            ...
            <div className="space-y-3">
                {displayData.activeServices.map(service => (
                    <div key={service.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm relative group hover:border-teal-300 transition-colors">
                        <div className="flex justify-between items-start">
                            <div className="flex items-center">
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center mr-3 ${
                                    service.id === CareCategory.ELDERLY ? 'bg-green-100 text-green-600' :
                                    service.id === CareCategory.CHILDREN ? 'bg-slate-100 text-slate-600' :
                                    service.id === CareCategory.PETS ? 'bg-orange-100 text-orange-600' : 'bg-blue-100 text-blue-600'
                                }`}>
                                    <StarIcon className="w-5 h-5" /> {/* Placeholder for dynamic category icon if needed */}
                                </div>
                                <div>
                                    <h4 className="font-bold text-slate-800">{service.label}</h4>
                                    <p className="text-sm text-teal-600 font-semibold">{service.price}€ <span className="text-slate-400 font-normal">/ hora</span></p>
                                </div>
                            </div>
                            <button 
                ...
                {displayData.activeServices.length === 0 && (
                    <div className="text-center py-8 bg-white rounded-xl border-2 border-dashed border-slate-200">
                        <p className="text-slate-500 text-sm mb-3">No tienes servicios activos.</p>
                        <button 
                            onClick={() => onNavigateEditProfile(CareCategory.ELDERLY)} 
        ...
        <section>
            <h3 className="font-bold text-slate-800 mb-3 px-1">Cuenta y Seguridad</h3>

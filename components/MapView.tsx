...
const createProviderIcon = (provider: Provider, isSelected: boolean) => {
    ...
    if (provider.categories.length > 1) {
        ...
        const colors = provider.categories.map(cat => categoryColors[cat] || '#64748b');
    ...
    const badgeHtml = (provider.rating >= 4.8) ? `<div style="position:absolute; top:-5px; right:-5px; background:#f59e0b; color:white; border-radius:50%; width:20px; height:20px; font-size:10px; display:flex; align-items:center; justify-content:center; border:2px solid white; font-weight:bold; z-index:10;">★</div>` : '';
...
const MapView: React.FC<MapViewProps> = ({ providers, userLocation, onViewProfile, onBack, onLocationUpdate, onLocationLoading, onLocationError, currentUserPhotoUrl }) => {
  ...
  const displayPhotoUrl = currentUserPhotoUrl || 'https://images.unsplash.com/photo-1511367461989-f85a21fda167?q=80&w=200';
  ...
  useEffect(() => {
    const interval = setInterval(() => {
        ...
        if (Math.random() > 0.8) {
             setNotification({ text: '¡Nuevo cuidador disponible en tu zona!', visible: true });
  ...
  const handleMarkerClick = useCallback((provider: Provider) => {
    ...
    if (mapRef.current) {
        mapRef.current.flyTo([provider.coordinates.latitude, provider.coordinates.longitude], 16, { duration: 0.5 });
    }
  ...
  const handleShowRoute = () => {
      ...
      const latlngs = [
          ...
          [selectedProvider.coordinates.latitude, selectedProvider.coordinates.longitude]
      ];

      // Draw a simple dashed line
      const polyline = L.polyline(latlngs, { color: '#2DD4BF', weight: 4, dashArray: '10, 10', opacity: 0.8 }).addTo(mapRef.current);
  ...
  useEffect(() => {
    ...
    const filteredProviders = providers.filter(provider => {
      const categoryMatch = selectedCategory === 'all' || provider.categories.includes(selectedCategory);
  ...
  }, [isMapInitialized, providers, selectedCategory, searchQuery, selectedProvider, handleMarkerClick]);


  // Filter buttons configuration with specific colors to match markers
  const filterButtons = [
      { id: CareCategory.ELDERLY, icon: ElderlyIcon, label: 'Mayores', colorClass: 'text-green-500', bgActive: 'bg-green-500 ring-green-300' },
      { id: CareCategory.CHILDREN, icon: ChildIcon, label: 'Niños', colorClass: 'text-slate-500', bgActive: 'bg-slate-500 ring-slate-300' },
      { id: CareCategory.PETS, icon: PetIcon, label: 'Mascotas', colorClass: 'text-orange-500', bgActive: 'bg-orange-500 ring-orange-300' },
      { id: CareCategory.HOUSEKEEPING, icon: CleaningIcon, label: 'Limpieza', colorClass: 'text-blue-500', bgActive: 'bg-blue-500 ring-blue-300' },
      { id: 'all', icon: QueueListIcon, label: 'Todos', colorClass: 'text-slate-700', bgActive: 'bg-slate-800 ring-slate-300' }
  ];
  ...
  return (
    <div className="w-screen h-screen overflow-hidden relative bg-slate-200">
      ...
      <div className="absolute top-4 left-4 right-4 z-[1000] flex gap-3 pointer-events-none">
          <div className="bg-white rounded-full shadow-md p-3 flex items-center justify-center text-slate-600 pointer-events-auto cursor-pointer hover:bg-slate-50" onClick={onBack}>
      ...
      <div 
        className={`absolute right-4 z-[1000] pointer-events-auto transition-all duration-500 ease-out ${isSheetExpanded ? 'bottom-[42vh]' : 'bottom-48'}`}
      >
          <button
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-[0_-5px_20px_rgba(0,0,0,0.1)] z-[2000] p-4 pb-8 animate-slide-up-fast pointer-events-auto">
              <div className="flex gap-4">
                  <div className="relative">
                    <img src={selectedProvider.photoUrl} alt={selectedProvider.name} className="w-16 h-16 rounded-full object-cover border-2 border-white shadow-md" />
                    <div className={`absolute bottom-0 right-0 w-4 h-4 rounded-full border-2 border-white ${
                        selectedProvider.status === 'available' ? 'bg-green-500' : 
                        selectedProvider.status === 'busy' ? 'bg-red-500' : 'bg-slate-400'
                    }`}></div>
                  </div>
                  <div className="flex-1">
                      <div className="flex justify-between items-start">
                          <h3 className="font-bold text-lg text-slate-800">{selectedProvider.name}</h3>
                          <div className="flex items-center bg-amber-100 px-2 py-0.5 rounded-full">
                              <StarIcon className="w-3 h-3 text-amber-500 mr-1" />
                              <span className="text-xs font-bold text-amber-700">{selectedProvider.rating}</span>
                          </div>
                      </div>
                      <p className="text-sm text-slate-500 truncate">{selectedProvider.services.join(', ')}</p>
                      <div className="flex items-center mt-1">
                          <span className="font-bold text-teal-600 text-lg">{selectedProvider.hourlyRate}€</span>
              ...
              <button 
                onClick={() => { setSelectedProvider(null); if(routeLayerRef.current) { routeLayerRef.current.remove(); routeLayerRef.current = null; } }} 
      ...
      {!selectedProvider && (
         <div 
            ...
            className={`absolute bottom-0 left-0 right-0 bg-white rounded-t-3xl shadow-[0_-5px_20px_rgba(0,0,0,0.1)] z-[900] transition-all duration-500 cubic-bezier(0.32, 0.72, 0, 1) flex flex-col pointer-events-auto cursor-pointer ${isSheetExpanded ? 'h-[40vh] pt-3' : 'h-auto pb-8 pt-3'}`}
         >
            {/* Handle Bar */}
            ...
            <div className="w-full px-4 flex items-center justify-between mb-3 flex-shrink-0">
                <span className="text-lg font-bold text-slate-800">Lo último en la zona</span>
                <div className={`bg-slate-100 p-1 rounded-full transition-transform duration-300 ${isSheetExpanded ? 'rotate-180' : ''}`}>
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-6 hide-scrollbar">
                {/* Horizontal Pills (Always Visible at top) */}
                ...
                <div className={`space-y-4 transition-opacity duration-500 ${isSheetExpanded ? 'opacity-100' : 'opacity-0 pointer-events-none h-0'}`}>
                    <h4 className="text-sm font-bold text-slate-400 uppercase tracking-wider">Novedades y Alertas</h4>
                    ...
                    <div className="flex items-start p-3 bg-slate-50 rounded-xl border border-slate-100">
                        ...
                        <div>
                            <p className="font-semibold text-slate-800 text-sm">Aviso de tráfico en el centro</p>
                            <p className="text-xs text-slate-500 mt-1">Cortes por obras en Gran Vía. Los cuidadores podrían sufrir ligeros retrasos.</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-start p-3 bg-slate-50 rounded-xl border border-slate-100">
                        ...
                        <div>
                            <p className="font-semibold text-slate-800 text-sm">Nuevo parque para perros inaugurado</p>
                            <p className="text-xs text-slate-500 mt-1">Zona Retiro. Ideal para paseos largos y seguros con mascotas.</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-start p-3 bg-slate-50 rounded-xl border border-slate-100">
                        ...
                        <div>
                            <p className="font-semibold text-slate-800 text-sm">Actividades infantiles gratuitas</p>
                            <p className="text-xs text-slate-500 mt-1">Este fin de semana en la Plaza Mayor. Talleres de pintura y juegos.</p>
                        </div>
                    </div>
                </div>
            </div>
